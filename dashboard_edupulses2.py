...
def carregar_mapeamentos_salvos(nome_arquivo):
    mapeamentos = {}
    if os.path.exists(nome_arquivo):
        try:
            df_map = pd.read_csv(nome_arquivo)
            for _, row in df_map.iterrows():
                plano_corrigido = row['plano_corrigido'] if pd.notna(row['plano_corrigido']) else "Sem plano definido"
                periodicidade_corrigida = row['periodicidade_corrigida'] if pd.notna(row['periodicidade_corrigida']) else "Mensal" 
                if periodicidade_corrigida not in PERIODICIDADE_VALIDAS: 
                    periodicidade_corrigida = "Mensal"
                mapeamentos[row['descricao_original']] = {
                    'plano': plano_corrigido,
                    'periodicidade': periodicidade_corrigida
                }
        except Exception as e:
            st.sidebar.error(f"Erro ao carregar mapeamentos de '{nome_arquivo}': {e}")
    return mapeamentos
...
def calcular_mrr_arpa_mensal(df_faturas_processado):
    ...
    df_mrr_calc = df_faturas_processado[
        pd.notna(df_faturas_processado['valor_mensal_plano']) & 
        (df_faturas_processado['valor_mensal_plano'] > 0) &
...
# NOVA FUNÇÃO PARA COHORT DE LTV ACUMULADO
def calcular_cohort_ltv_acumulado_heatmap(df_faturas_pipeline, df_clientes_unicos_para_cohort, data_final_analise):
    ...
    for _, row_cliente in df_clientes_unicos_para_cohort.iterrows():
        ...
        faturas_do_cliente = df_faturas_pipeline[
            ...
            (df_faturas_pipeline['data_emissao'] >= data_entrada_cliente_dt) # Considerar faturas a partir da entrada
        ...
        while mes_atual_cohort_inicio_dt <= max_analise_dt:
            # Fim do mês atual do cohort para filtrar faturas
            fim_do_mes_atual_cohort = mes_atual_cohort_inicio_dt + pd.offsets.MonthEnd(0)
            
            # Faturas do cliente emitidas ATÉ o fim deste mês do cohort
            ...
            cohort_ltv_data.append({
                'safra_periodo': safra,
                'mes_desde_entrada': mes_desde_entrada_val,
                'documento_cliente': doc, # Para poder calcular a média depois
                'ltv_acumulado_cliente': ltv_acumulado_neste_ponto
            })
            
            # Condição de parada para este cliente: se o LTV acumulado não mudar mais E não for o mês 0,
            # ou se a data da última fatura do cliente for antes do início do próximo mês do cohort.
            ...
            if pd.notna(data_ultima_fatura_cliente_dt) and data_ultima_fatura_cliente_dt < (mes_atual_cohort_inicio_dt + pd.offsets.MonthBegin(1)):
                if mes_desde_entrada_val > 0 and ltv_acumulado_neste_ponto == faturas_do_cliente['valor_total'].sum(): # Chegou ao LTV total do cliente
                    break


            mes_atual_cohort_inicio_dt = mes_atual_cohort_inicio_dt + pd.offsets.MonthBegin(1)
            mes_desde_entrada_val += 1
            if mes_desde_entrada_val > 24: break # Limite de visualização
...
if not st.session_state.mapeamentos_carregados_do_arquivo:
    mapeamentos_salvos_dict = carregar_mapeamentos_salvos(NOME_ARQUIVO_MAPEAMENTO)
...
arquivo_base_b2b = st.sidebar.file_uploader("Carregue a base B2B (.csv ou .xlsx)", type=["csv", "xlsx", "xls"], key="uploader_base")
...
data_inicio_default_filtro = None
data_fim_default_filtro = None
min_data_permitida_filtro = None
max_data_permitida_filtro = None
opcoes_tipo_cliente_filtro = ["Todos"]
opcoes_plano_filtro = ["Todos"]
...
if st.session_state.df_pipeline_output is not None and not st.session_state.df_pipeline_output.empty:
...
if st.session_state.df_pipeline_output is not None:
    ...
    if data_inicio_selecionada and data_fim_selecionada:
        data_inicio_ts = pd.Timestamp(data_inicio_selecionada)
        data_fim_ts = pd.Timestamp(data_fim_selecionada) 
    ...
    if tipo_cliente_selecionado_filtro_val != "Todos":
        df_filtrado_para_kpis = df_filtrado_para_kpis[df_filtrado_para_kpis['tipo_cliente'] == tipo_cliente_selecionado_filtro_val]
    if plano_selecionado_filtro_val != "Todos":
        df_filtrado_para_kpis = df_filtrado_para_kpis[df_filtrado_para_kpis['nome_plano_extraido'] == plano_selecionado_filtro_val]
    if periodicidade_selecionada_filtro_val != "Todos":
        df_filtrado_para_kpis = df_filtrado_para_kpis[df_filtrado_para_kpis['periodicidade_extraida'] == periodicidade_selecionada_filtro_val]
    if status_selecionado_filtro_val != "Todos":
        df_filtrado_para_kpis = df_filtrado_para_kpis[df_filtrado_para_kpis['status_cliente'] == status_selecionado_filtro_val]
    ...
    **Como é calculado:**
    ...
    if not df_filtrado_para_kpis.empty:
        ...
        if not df_mrr_calculado_kpi.empty:
            ...
            mrr_display_val = "N/A"; arpa_display_val = "N/A"
            if not dados_ultimo_mes_fechado_mrr.empty:
                ...
                mrr_display_val = f"R$ {mrr_ultimo_mes_fechado:,.2f}"; arpa_display_val = f"R$ {arpa_ultimo_mes_fechado:,.2f}"
            ...
            fig_mrr = px.line(df_mrr_calculado_kpi, x='MesAnoStr', y='MRR_Total', title="Evolução do MRR Total", 
                              labels={'MesAnoStr': 'Mês', 'MRR_Total': 'MRR (R$)'}, markers=True, text='MRR_Total')
            ...
            if 'ARPA' in df_mrr_calculado_kpi.columns and not df_mrr_calculado_kpi['ARPA'].isnull().all() and (df_mrr_calculado_kpi['ARPA'].fillna(0) != 0).any():
                fig_arpa = px.line(df_mrr_calculado_kpi, x='MesAnoStr', y='ARPA', title="Evolução da ARPA", 
                                   labels={'MesAnoStr': 'Mês', 'ARPA': 'ARPA (R$)'}, markers=True, text='ARPA')
        ...
        if not df_permanencia_clientes_calc.empty and 'permanencia_meses' in df_permanencia_clientes_calc.columns:
            ...
            if 'data_entrada_cliente' in df_permanencia_clientes_calc.columns and not df_permanencia_clientes_calc['data_entrada_cliente'].dropna().empty:
                ...
                if not cohort_data_permanencia.empty:
                    fig_cohort_permanencia = px.bar(cohort_data_permanencia.sort_values(by='safra'), 
                                                    x='safra', y='Permanencia_Media_Meses',
                                                    text='Permanencia_Media_Meses', 
                                                    title='Permanência Média por Safra de Entrada (Meses)')
                    ...
                    st.info("Não há dados de safra suficientes para o gráfico de cohort de permanência.")
            else:
                st.info("Coluna 'data_entrada_cliente' ausente ou vazia para gerar cohort de permanência.")
            ...
            **Como é calculado:** Para cada safra e cada "mês desde a entrada": (Clientes da safra ainda ativos no mês X) / (Total de clientes na safra inicial).
            ...
            if not df_filtrado_para_kpis.empty:
                ...
                df_clientes_unicos_para_heatmap = df_filtrado_para_kpis.drop_duplicates(subset=['documento_cliente'])[[
                    'documento_cliente', 'data_entrada_cliente', 'status_cliente', 'data_churn_cliente'
                ]].copy()

                heatmap_data_retencao, safra_initial_size_map_retencao = calcular_cohort_retencao_heatmap(
                    st.session_state.df_pipeline_output, 
                    df_clientes_unicos_para_heatmap, 
                    data_final_heatmap_calc 
                ) 
                ...
                if not heatmap_data_retencao.empty:
                    ...
                    for safra_idx, safra_nome in enumerate(heatmap_data_retencao.index):
                        ...
                        for mes_idx, mes_nome_col in enumerate(heatmap_data_retencao.columns):
                            ...
                            if pd.notna(valor_celula):
                                if mes_idx == 0: 
                                    row_text.append(f"{valor_celula:.0f}%<br>(N={tamanho_inicial})")
                                elif valor_celula > 0 : 
                                    row_text.append(f"{valor_celula:.0f}%")
                                else:
                                    row_text.append("") 
                            else:
                                row_text.append("") 
                    ...
                    fig_heatmap_retencao = go.Figure(data=go.Heatmap(
                        z=heatmap_data_retencao.values,
                        x=heatmap_data_retencao.columns,
                        y=heatmap_data_retencao.index,
                        colorscale='Greens', 
                        text=z_text, 
                        texttemplate="%{text}",
                        hoverongaps=False,
                    ...
                    fig_heatmap_retencao.update_layout(
                        title='Taxa de Retenção de Clientes por Safra',
                        xaxis_title='Meses Desde a Entrada',
                        yaxis_title='Safra de Entrada (Ano-Mês)',
                        yaxis_autorange='reversed',
                        height=max(400, len(heatmap_data_retencao.index) * 35 + 100),
                    )
                    st.plotly_chart(fig_heatmap_retencao, use_container_width=True)
                else:
                    st.info("Não foi possível gerar o heatmap de retenção com os dados e filtros atuais.")
            else:
                st.info("Dados de permanência não disponíveis para o heatmap de retenção.")
    ...
    **Cohort:** LTV médio por safra de entrada.
    """)
    if not df_filtrado_para_kpis.empty and \
       'documento_cliente' in df_filtrado_para_kpis.columns and \
       'total_gasto_cliente' in df_filtrado_para_kpis.columns and \
       'data_entrada_cliente' in df_filtrado_para_kpis.columns: 
        ...
        if not df_clientes_unicos_ltv.empty:
            ...
            if not df_clientes_unicos_ltv['data_entrada_cliente'].dropna().empty:
                df_ltv_cohort_calc_ltv = df_clientes_unicos_ltv.copy() # Renomeado para evitar conflito de nome
                ...
                if not cohort_data_ltv.empty:
                    fig_cohort_ltv = px.bar(cohort_data_ltv.sort_values(by='safra'), 
                                            x='safra', y='LTV_Medio_Safra',
                                            text='LTV_Medio_Safra', 
                                            title='LTV Médio por Safra de Entrada (R$)')
            ...
            **Como é calculado:** Para cada safra e "mês desde a entrada": Soma da receita (`valor_total` das faturas) gerada pelos clientes da safra até aquele mês X, dividido pelo total de clientes na safra inicial.
            ...
            """)
            if not df_filtrado_para_kpis.empty: # df_filtrado_para_kpis contém todas as faturas
                ...
                # Usar df_clientes_unicos_ltv para obter 'data_entrada_cliente' e 'documento_cliente' das safras
                heatmap_data_ltv_acum, safra_initial_size_map_ltv = calcular_cohort_ltv_acumulado_heatmap(
                    df_filtrado_para_kpis, # Passa todas as faturas filtradas
                    df_clientes_unicos_ltv, # Passa os clientes únicos da seleção atual
                    data_final_heatmap_ltv
                )
                ...
                if not heatmap_data_ltv_acum.empty:
                    ...
                    for safra_idx_ltv, safra_nome_ltv in enumerate(heatmap_data_ltv_acum.index):
                        ...
                        for mes_idx_ltv, mes_nome_col_ltv in enumerate(heatmap_data_ltv_acum.columns):
                            ...
                            if pd.notna(valor_celula_ltv):
                                if mes_idx_ltv == 0: # M0
                                    row_text_ltv.append(f"R${valor_celula_ltv:,.0f}<br>(N={tamanho_inicial_ltv})")
                                elif valor_celula_ltv > 0 : 
                                    row_text_ltv.append(f"R${valor_celula_ltv:,.0f}")
                                else:
                                    row_text_ltv.append("") 
                            else:
                                row_text_ltv.append("") 
                    ...
                    fig_heatmap_ltv = go.Figure(data=go.Heatmap(
                        z=heatmap_data_ltv_acum.values,
                        x=heatmap_data_ltv_acum.columns,
                        y=heatmap_data_ltv_acum.index,
                        colorscale='Blues', # Escala de cor para LTV
                        text=z_text_ltv, 
                        texttemplate="%{text}",
                        hoverongaps=False,
                        xgap=1, ygap=1 
                    ))
                    fig_heatmap_ltv.update_layout(
                        title='LTV Médio Acumulado por Safra',
                        xaxis_title='Meses Desde a Entrada',
                        yaxis_title='Safra de Entrada (Ano-Mês)',
                        yaxis_autorange='reversed',
                        height=max(400, len(heatmap_data_ltv_acum.index) * 35 + 100),
                    )
                    st.plotly_chart(fig_heatmap_ltv, use_container_width=True)
                else:
                    st.info("Não foi possível gerar o heatmap de LTV acumulado com os dados e filtros atuais.")
            else:
                st.info("Dados de LTV não disponíveis para o heatmap.")

        else:
            st.info("Não há clientes únicos suficientes na seleção para calcular LTV.")
    else:
        st.info("Nenhum dado ou colunas necessárias encontradas com os filtros selecionados para calcular LTV.")
    ...
    **Cálculo:** `data_churn_cliente` é a data de fim da fatura que não foi renovada. `Ativos no Início do Mês Estimado` vem do cálculo do MRR.
    """)
    df_mrr_para_taxa_churn = df_mrr_calculado_kpi 
    if not df_filtrado_para_kpis.empty:
        ...
        if data_fim_selecionada: 
            ...
            if pd.notna(max_data_emissao_filtrada):
                 data_final_base_para_churn_recente = max_data_emissao_filtrada
        
        df_churn_mensal_calculado = calcular_metricas_churn_mensal(df_clientes_unicos_para_churn_kpi, 
                                                                  data_final_base_para_churn_recente, 
                                                                  df_mrr_para_taxa_churn) 
        ...
        if not df_churn_mensal_calculado.empty:
            ...
            churn_count_ultimo_mes_val = 0
            churn_rate_ultimo_mes_val = 0.0 
            ...
            fig_churn_qtd = px.bar(df_churn_mensal_calculado, x='MesAnoStr', y='Churn_Count', 
                                   title="Evolução da Quantidade de Churn Mensal",
                                   labels={'MesAnoStr': 'Mês', 'Churn_Count': 'Nº de Clientes Churned'},
                                   text='Churn_Count') 
            fig_churn_qtd.update_traces(texttemplate='%{text}', textposition='outside') 
            st.plotly_chart(fig_churn_qtd, use_container_width=True)

            fig_churn_taxa = px.line(df_churn_mensal_calculado, x='MesAnoStr', y='Taxa_Churn_Mensal', 
                                   title="Evolução da Taxa de Churn Mensal (%)",
                                   labels={'MesAnoStr': 'Mês', 'Taxa_Churn_Mensal': 'Taxa de Churn (%)'}, markers=True,
                                   text='Taxa_Churn_Mensal') 
            ...
            st.info("Não há dados de churn para exibir com os filtros atuais.")
        ...
        churns_recentes_df_calc = df_clientes_unicos_para_churn_kpi[
            (df_clientes_unicos_para_churn_kpi['status_cliente'] == 'Inativo (Churned)') &
            (pd.to_datetime(df_clientes_unicos_para_churn_kpi['data_churn_cliente']) >= data_inicio_churn_recente_calc) &
            (pd.to_datetime(df_clientes_unicos_para_churn_kpi['data_churn_cliente']) <= data_final_base_para_churn_recente)
        ].copy() 
        ...
        if not churns_recentes_df_calc.empty:
            ...
            st.info(f"Nenhum cliente churnou entre {data_inicio_churn_recente_calc.strftime('%d/%m/%Y')} e {data_final_base_para_churn_recente.strftime('%d/%m/%Y')}.")
    else:
        st.info("Nenhum dado encontrado com os filtros selecionados para calcular Churn.")
    ...
    **Como é calculado (mês a mês):**
    ...
    """)
    if not df_filtrado_para_kpis.empty:
        df_clientes_unicos_para_dinamica = df_filtrado_para_kpis.drop_duplicates(subset=['documento_cliente'])[[
            'documento_cliente', 'data_entrada_cliente', 'data_churn_cliente', 'status_cliente'
        ]]
        ...
        if data_fim_selecionada:
            ...
            if pd.notna(max_data_emissao_filtrada_dinamica):
                 data_final_base_dinamica = max_data_emissao_filtrada_dinamica

        df_dinamica_assinantes_calc = calcular_dinamica_assinantes_mensal(
            df_filtrado_para_kpis, 
            df_clientes_unicos_para_dinamica, 
            data_final_base_dinamica
        )
        ...
        if not df_dinamica_assinantes_calc.empty:
            ...
            df_dinamica_plot = df_dinamica_plot_prep.melt(
                id_vars=['MesAnoStr'], 
                value_vars=['Novos', 'Ativos_Legado', 'Cancelados_Plot_Val'], 
                var_name='Tipo_Movimentacao_Melt', 
                value_name='Numero_Clientes'
            )
            
            def definir_legenda_e_rotulo(row):
                if row['Tipo_Movimentacao_Melt'] == 'Cancelados_Plot_Val':
                    return 'Cancelados', abs(row['Numero_Clientes'])
            ...
            df_dinamica_plot['Tipo_Movimentacao_Legenda'] = res[0]
            df_dinamica_plot['Texto_Rotulo'] = res[1].astype(int)
            ...
            fig_dinamica = px.bar(df_dinamica_plot, 
                                  x='MesAnoStr', 
                                  y='Numero_Clientes', 
                                  color='Tipo_Movimentacao_Legenda',
                                  title="Dinâmica Mensal de Assinantes",
                                  labels={'MesAnoStr': 'Mês', 'Numero_Clientes': 'Número de Clientes', 'Tipo_Movimentacao_Legenda': 'Status'},
                                  color_discrete_map={ 
                                        'Novos': 'mediumseagreen', 
                                        'Ativos_Legado': 'royalblue', 
                                        'Cancelados': 'indianred' 
                                  },
                                  text='Texto_Rotulo' 
                                 )
            ...
            st.info("Não há dados suficientes para gerar o gráfico de dinâmica de assinantes.")
    else:
        st.info("Nenhum dado encontrado com os filtros selecionados para a dinâmica de assinantes.")
    ...
    **Como é calculado:** Para clientes 'Inativo (Churned)', o `valor_mensal_plano` de sua última fatura ativa é somado no mês da `data_churn_cliente`.
    ...
    if not df_filtrado_para_kpis.empty:
        ...
        if data_fim_selecionada:
             data_final_base_rec_churn = pd.Timestamp(data_fim_selecionada)
        elif 'data_emissao' in df_filtrado_para_kpis and not df_filtrado_para_kpis['data_emissao'].dropna().empty:
            max_data_emissao_filtrada_rec_churn = df_filtrado_para_kpis['data_emissao'].max()
            if pd.notna(max_data_emissao_filtrada_rec_churn):
                 data_final_base_rec_churn = max_data_emissao_filtrada_rec_churn
        ...
        if not df_receita_churn_calc.empty:
            fig_receita_churn = px.bar(df_receita_churn_calc, x='MesAnoStr', y='Receita_Perdida_Churn',
                                     title="Evolução da Receita Perdida por Churn (Mensal)",
                                     labels={'MesAnoStr': 'Mês do Churn', 'Receita_Perdida_Churn': 'Receita Perdida (R$)'},
                                     text='Receita_Perdida_Churn')
            ...
            st.info("Não há dados de receita churn para exibir com os filtros atuais.")
    else:
        st.info("Nenhum dado encontrado com os filtros selecionados para calcular Receita Churn.")


    # --- Seção de Ajuste ---
    st.subheader("🔧 Ajustar Classificações de Serviço")
    with st.expander("Clique aqui para ajustar classificações"):
        ...
        df_para_ajuste_ui = pd.concat([df_para_ajuste_ui_parte1, df_para_ajuste_ui_parte2]).drop_duplicates().reset_index(drop=True)
        ...
        if not df_para_ajuste_ui.empty:
            ...
            num_descricoes_a_mostrar = st.slider("Número de descrições para ajustar/revisar por vez:", 1, max(1, min(5,len(descricoes_unicas_para_ajuste))), 
                                                 value=min(5, len(descricoes_unicas_para_ajuste)), 
                                                 key="slider_num_desc_v7_filtros_texto_info_completo_cohort_heatmap_LTV_final") 
            st.write(f"Exibindo {len(descricoes_unicas_para_ajuste[:num_descricoes_a_mostrar])} de {len(descricoes_unicas_para_ajuste)} descrições para ajuste/revisão:")

            correcoes_atuais_formulario = {} 
            form_correcao = st.form(key="form_correcao_servicos_v7_filtros_texto_info_completo_cohort_heatmap_LTV_final") 
            with form_correcao:
                for i, desc_servico in enumerate(descricoes_unicas_para_ajuste[:num_descricoes_a_mostrar]):
                    st.markdown(f"**Descrição:** `{desc_servico}`")
                    
                    item_df = df_base_para_ajuste[df_base_para_ajuste['descricao_servico'] == desc_servico].drop_duplicates(subset=['nome_cliente', 'valor_total'])
                    exemplos_info = []
                    ...
                    plano_sugerido_no_df = item_df['nome_plano_extraido'].iloc[0] if not item_df.empty else "Sem plano definido"
                    periodicidade_sugerida_no_df = item_df['periodicidade_extraida'].iloc[0] if not item_df.empty else "Mensal" 
                    ...
                    key_plano = f"plano_{i}_{chave_base_widget}_v4_cohort_heatmap_LTV_final" 
                    key_period = f"period_{i}_{chave_base_widget}_v4_cohort_heatmap_LTV_final"
    ...
    if st.session_state.correcoes_aplicadas_map:
        ...
        for k, v_dict in st.session_state.correcoes_aplicadas_map.items():
            ...
            if (plano_val and plano_val != "Sem plano definido") or \
               (period_val and period_val in PERIODICIDADE_VALIDAS) :
                map_df_list.append({
        ...
        if map_df_list:
            ...
            csv_map = map_df.to_csv(index=False).encode('utf-8-sig')
            st.sidebar.download_button(
                label="Baixar Mapeamentos (.csv)", data=csv_map, file_name=NOME_ARQUIVO_MAPEAMENTO, mime='text/csv',
            )
            st.sidebar.caption(f"Salve como '{NOME_ARQUIVO_MAPEAMENTO}' na pasta do script.")
        else:
            st.sidebar.info("Nenhuma correção substancial no mapa da sessão para salvar.")
    else:
        st.sidebar.info("Nenhuma correção no mapa da sessão para salvar.")

    st.subheader("Resumo da Extração (Filtrado)") 
